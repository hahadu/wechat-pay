...
use Hahadu\WechatPay\Kernel\WxPayData\WxPayResults;
use Hahadu\WechatPay\Kernel\WxPayData\WxPayOrderQuery;
use Hahadu\WechatPay\Kernel\WxPayData\WxPayCloseOrder;
use Hahadu\WechatPay\Kernel\WxPayData\WxPayRefund;
use Hahadu\WechatPay\Kernel\WxPayData\WxPayRefundQuery;
use Hahadu\WechatPay\Kernel\WxPayData\WxPayDownloadBill;
use Hahadu\WechatPay\Kernel\WxPayData\WxPayMicroPay;
use Hahadu\WechatPay\Kernel\WxPayData\WxPayReverse;
...
class WxPayApi
{
     ...
     * @return array|false 成功时返回，其他抛异常
     */
    public static function unifiedOrder($config, $inputObj, $timeOut = 6)
    {
        $url = $config->GetApiUrl()."/pay/unifiedorder";
        //检测必填参数
    ...
    {
        $url = $config->GetApiUrl()."/pay/orderquery";
        //检测必填参数
    ...
    {
        $url = $config->GetApiUrl()."/pay/closeorder";
        //检测必填参数
        if(!$inputObj->IsOut_trade_noSet()) {
    ...
    {
        $url = $config->GetApiUrl()."/secapi/pay/refund";
        //检测必填参数
        if(!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
    ...
    {
        $url = $config->GetApiUrl()."/pay/refundquery";
        //检测必填参数
    ...
    {
        $url = $config->GetApiUrl()."/pay/downloadbill";
        //检测必填参数
    ...
    {
        $url = $config->GetApiUrl()."/pay/micropay";
        //检测必填参数
    ...
    {
        $url = $config->GetApiUrl()."/secapi/pay/reverse";
        //检测必填参数
        if(!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
    ...
    {
        $url = $config->GetApiUrl()."/payitil/report";
        //检测必填参数
     ...
     * @param int $timeOut
     * @throws WxPayException
     * @return bool|string|mixed 成功时返回，其他抛异常
     */
